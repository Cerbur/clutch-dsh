...
- Main 与 Worktree group row 的 trailing action rail 在静止时保持零宽度，不为 Dashboard、menu 或 Session `+` 预留空间；hover、focus 或打开 menu 时才按实际可用控件的 intrinsic width 展示，过长的 Worktree label 在悬浮时自动横向滚动，鼠标离开后回到开头；
