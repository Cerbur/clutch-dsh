...
import type { WorktreeWorkspaceView } from '../view/worktree-view.js';
import type { WorkspaceLike, WorktreeTranslate } from './types.js';
    ...
    .filter((view) =>
      view.bindings.some(
        (binding) =>
          ...
          view.worktrees.some(
            (record) => record.worktreeId === binding.worktreeId && record.status === 'removed',
          ),
      ),
    )
...
export function workspaceMatches(
  ...
  if (
    view.worktrees.some(
      (record) => includesText(record.branch, query) || includesText(record.absolutePath, query),
    )
  ) {
    return true;
  }
...
export function resolveCurrentSessionLocation(
  ...
  const worktree = view.worktrees.find(
    (candidate) =>
...
export function currentSessionRevealKeys(
  ...
  return [
    ...
    ...(location.kind === 'worktree' && location.archived
      ? ['archived:' + location.workspaceId]
      : []),
...
export function isSessionGroupAutoExpanded(
  ...
  return (
    currentSessionRevealActive && shouldRevealCurrentSessionGroup(sessionIds, currentSessionId)
  );
}
