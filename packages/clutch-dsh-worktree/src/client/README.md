...
- Session menus retain Rename/Fork/Archive for ordinary Sessions, and Session drag ordering is restricted to the current visual Main or Worktree group. A provisional blank Session is visible only while it is the current DSH Session, uses the localized `New Session` label, and has no Session action menu; the binding remains browser/sidecar-owned even when the row is hidden.
