...
回调，不再维护一套独立的 JSX 或视觉样式。Worktree 的 remove menu、确认弹窗
和状态继续保留在同一个组件中，由调用方通过参数决定是否透出；Main 不传入该
参数，因此不显示 Worktree remove 选项。

底层 `WorktreeManager.removeWorktree` contract 和实现不变，避免把本次展示调整
扩大成 API 变更。
...
5. 可选地渲染 group menu；active Worktree 传入 remove menu，Main 和 detached
   Worktree 不传入。
...
| Active Worktree | branch 名称 | `IconBranchOutline16` | ready/repair | 新增 Worktree Session | Remove Worktree |
...
- Main 的 branch/tree icon 与普通 Worktree 位于同一 leading icon slot，保持行内
  对齐。
- active Worktree 的 remove menu、remove confirmation dialog、对应的 React
  state 和 UI handler 继续可达；Main 和 detached Worktree 不传 menu 参数，
  因此不显示 Worktree remove 选项。
...
- Worktree 查询、health projection、Session 列表、重试错误和 binding recovery
  保持现状。
- `removeWorktree` 由 active Worktree 的 Client menu 通过 Manager/API 调用；
  Main 和 detached 行不暴露该入口。
...
3. active Worktree 包含 remove Worktree menu、remove modal 和对应的 remove
   UI label，Main 和 detached 行不包含该入口。
4. Main 和普通 Worktree 仍保留正确的 `+` 行为、各自展开状态和菜单参数边界。
...
README 和 Client README 记录 active Worktree 可通过 remove menu 和确认弹窗删除
Worktree；Main 与 detached 行不显示该入口。同时保留 detached Worktree 的展示
语义，以及删除能力通过 Manager contract 执行这一实现边界。
