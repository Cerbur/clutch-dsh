...
实现基线为 `e6a263405af7c53d3909f5a325bdf83fa6064a13`。本文相对路径均相对该 worktree 根目录；任务内命令若使用 `src/`、`test/`，先进入 package 目录：
...
| `packages/clutch-dsh-worktree/src/host/service.ts`、`src/host/dsh-host-read-adapter.ts`（均位于 package 内） | Host composition、只读 adapter、dispose；Task 1 |
...
- Modify: `packages/clutch-dsh-worktree/src/host/dsh-host-read-adapter.ts`
...
- [ ] **Step 2: 添加并运行当前会失败的菜单与真实 Host 用例。**

在 selectors 测试导入新函数，加入：

```js
test('archive menus block busy, unknown and pending activity', () => {
  assert.equal(selectors.worktreeActivityBlockReason({ state: 'busy', sessionIds: ['s1'] }, false), 'busy');
...
```ts
const ids = [...new Set(snapshot.bindings
...
test('archive clean preflight does not manufacture a transaction', async () => {
  await withGitFixture(async ({ provider, sidecar, workspaceRoot }) => {
    const record = await provider.createWorktree({
      workspaceId: 'ws_one', branch: 'main', newBranch: 'feature/preflight-evidence',
    });
    ...
    await assert.rejects(provider.cleanWorktree({
      workspaceId: 'ws_one', worktreeId: record.worktreeId, mutationToken: token,
...
const removable = issue.operationId === undefined
...
- [ ] **Step 3: 写入保守恢复及 issue 清除回归，再修改恢复分支。**
...
```ts
if (pending.type === 'clean-worktree'
    && pending.phase === 'recovery-needed'
    && !exact && !pendingTargetExists) {
...
const {
  ...
  recoveryIssues: oldRecoveryIssues,
  ...stableFields
} = snapshot;
void oldRepository;
...
- [ ] **Step 4: 运行 Provider/Manage 回归并记录证据。**
...
- [ ] **Step 1: 新增 context 及 reveal 回归。**

在 client-context.test.mjs 的现有 baseInput fixture 中加入：

```js
test('archived ready binding retains Worktree context', () => {
  const context = resolveWorktreeSessionContext(baseInput({
    worktrees: [{
      worktreeId: 'wt1', workspaceId: 'ws1', absolutePath: '/tmp/wt1',
      branch: 'feature/context', source: 'external', status: 'removed', health: 'ready',
    }],
    bindings: [{
      worktreeId: 'wt1', workspaceId: 'ws1', sessionId: 'session-1', status: 'active',
    }],
  }));
...
test('archived current Session reveals its archive ancestor', () => {
  assert.deepEqual(selectors.currentSessionRevealKeys({
    sessionId: 's1', workspaceId: 'ws1', worktreeId: 'wt1',
    groupKey: 'worktree:wt1', kind: 'worktree', archived: true,
...
test('forget prevents a late binding failure from reviving fork recovery', async () => {
  ...
  const coordinator = createWorktreeSessionForkCoordinator({
    ...
    bindSession: () => {
      signalStarted();
      return new Promise((_resolve, reject) => { rejectBind = reject; });
    },
  });
  try {
    ...
    coordinator.forgetWorktree({
      workspaceId: 'workspace-one', worktreeId: 'worktree-one',
...
const versionOf = (versions: Map<string, number>, key: string) =>
  versions.get(key) ?? 0;
```
...
- [ ] **Step 3: 接通 Surface、entry 和 projection 清理。**
...
test('committed cleanup refreshes even when permission normalization rejects', async () => {
  ...
  await runWorktreeCleanupFlow({
    ...
    onCommitted: () => { calls.push('committed'); },
    refresh: async () => { calls.push('refresh'); },
    normalize: async () => { calls.push('permission'); throw failure; },
    isCurrent: () => true,
...
- [ ] **Step 2: 将 commit 与两个 follow-up 分离。**

在新模块定义上述 interface 并实现：

```ts
export async function runWorktreeCleanupFlow(
  input: WorktreeCleanupFlowInput,
): Promise<void> {
  await input.clean();
  if (!input.isCurrent()) return;
  input.onCommitted();
  const follow = async (
    stage: 'refresh' | 'permission',
  ...
  await Promise.all([
    follow('refresh', input.refresh),
    follow('permission', input.normalize),
  ]);
}
```
...
- [ ] **Step 3: Surface 应用已提交 projection，接入独立重试。**
...
- [ ] **Step 4: 验证实际 action 和 Surface 接线。**
