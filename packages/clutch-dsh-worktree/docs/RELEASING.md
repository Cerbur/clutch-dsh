...
| package DSH peer range  | 所有 `@deepseek-ai/dsh-*` peer 使用 `>=0.1.5-rc.1 || >=0.1.7-rc.2`                                          |
| local validation graph  | 所有对应 `@deepseek-ai/dsh-*` dev dependency 固定 `0.1.7-rc.2`；Cordis peer `^4.0.1`、dev `4.0.4`            |
| unsupported graph       | `dsh-v0.1.5-rc.1` 之前的版本，包括 `dsh-v0.1.1-rc.2`；早期 graph 不满足当前 Client 和 WorkspaceSource contract |
...
- 发布前确认 DSH compatibility floor 仍为 `dsh-v0.1.5-rc.1`，并保留 `dsh-v0.1.7-rc.2` 的 prerelease comparator；运行 package manifest、Client
