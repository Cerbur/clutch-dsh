...
for (const [shape, persistedEntry] of [['header', header], ['snapshot', snapshot]]) {
  test(`reads ${shape} entries without touching DSH-owned content`, async () => {
    const workspace = {
      id: 'ws_example',
      path: '/tmp/project',
      title: 'Project',
    ...
    const persistedHeaders = [
      persistedEntry('session_live', '/tmp/stale-live-copy'),
      persistedEntry('session_cold', '/tmp/project'),
      persistedEntry('session_without_cwd'),
    ];
