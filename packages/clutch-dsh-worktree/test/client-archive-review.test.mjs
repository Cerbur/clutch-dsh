...
import { hasOngoingSession } from '../lib/client/session-view.js';
import { bindingIdsFor } from '../lib/client/worktree-surface-selectors.js';
import { filterArchivedSessionIds } from '../lib/client/worktree-view.js';
import { createWorktreeRefreshGuard } from '../lib/client/worktree-view-read.js';
...
test('Archived group ignores native archived activity but retains hidden live activity', () => {
  const row = find((node) => ts.isJsxSelfClosingElement(node) && node.attributes.properties.some(
    (attr) => ts.isJsxAttribute(attr) && attr.name.text === 'kind' && attr.initializer?.text === 'archived-group',
  ));
  const expression = row.attributes.properties.find((attr) => attr.name?.text === 'hasOngoingSession').initializer.expression;
  const context = {
    hasOngoingSession, bindingIdsFor, filterArchivedSessionIds,
...
function permissionFixture(normalize) {
  const notice = { workspaceId: 'ws', worktreeId: 'wt', result: { status: 'unverified', retryable: true } };
  const record = { workspaceId: 'ws', worktreeId: 'wt', status: 'removed', diskCleanup: 'completed' };
  const published = [];
  const context = {
    React, styles: {}, t: (key) => key,
    ...
    mode: 'worktree', actionPending: false,
    setActionPending: (value) => { context.actionPending = value; },
  ...
  const declaration = (name) => find((node) => ts.isVariableDeclaration(node) && node.name.getText(ast) === name);
  const noticeJsx = find((node) => ts.isJsxExpression(node) && ts.isBinaryExpression(node.expression ?? {}) &&
...
test('permission retry rejection remains retryable without repeating disk cleanup', async () => {
  const fixture = permissionFixture(async () => { throw new Error('offline'); });
...
test('permission retry drops late results after forget or disposal and coalesces repeated clicks', async () => {
  for (const invalidate of [
    (context) => { context.readStateRef.current.views[0].worktrees = []; },
    (context) => { context.cleanupGuard.current.invalidate(); },
    (context) => { context.permissionNotice.getSnapshot = () => undefined; },
  ]) {
    let resolve;
    let calls = 0;
