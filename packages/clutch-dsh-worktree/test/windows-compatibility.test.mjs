...
test('persists a sidecar after writable temporary-file sync', async () => {
...
test('surfaces a remaining temporary-file sync failure', async () => {
  const dshHome = await mkdtemp(path.join(os.tmpdir(), 'clutch-dsh-worktree-windows-'));
  ...
  const syncMock = mock.method(fileHandlePrototype, 'sync', async () => {
    throw Object.assign(new Error('simulated Windows fsync failure'), { code: 'EPERM' });
  });
...
test('shares Windows path identity rules between browser-safe and Node layers', () => {
  ...
  const extendedVolume = '\\\\?\\Volume{GUID}\\Repo';
  ...
  assert.equal(normalizeWindowsPath(extendedVolume), '//?/volume{guid}/repo');
  assert.equal(sameWindowsPath(extendedVolume, 'volume{guid}/repo'), false);
  assert.equal(sameLexicalPath(extendedDrive, ordinaryDrive), true);
});

test('does not turn non-missing path errors into lexical matches', async () => {
  const dshHome = await mkdtemp(path.join(os.tmpdir(), 'clutch-dsh-worktree-path-identity-'));
  const filePath = path.join(dshHome, 'not-a-directory');
  await writeFile(filePath, 'probe');

  try {
    await assert.rejects(
      samePhysicalPath(path.join(filePath, 'left'), path.join(filePath, 'right')),
