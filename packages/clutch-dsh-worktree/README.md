...
The tree keeps native DSH Workspace presentation: clicking a Workspace or
Worktree row body toggles its browser-local expansion state, while the folder or
branch icon changes to a centered 18px chevron only on hover. Workspace,
Worktree, and Session copy follows the native tree text metrics, and the
Workspace, Main, and Worktree `+` controls share a fixed right-aligned action
