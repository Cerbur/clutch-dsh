...
Git 与变更使用受页面 viewport 限制的固定尺寸布局。宽度足够时，提交与变更文件在左列上下排列，中间是
可拖动的分割线，Diff 占满右列高度；拖动分割线，或聚焦后按方向键，即可在两个面板之间分配高度，
双击分割线恢复默认比例。宽度不足时保持原来的两行布局：第一行左右排布提交和变更文件，第二行展示
汇总 Diff。较长的 commit list 和变更文件 list 会在各自栏内滚动，不会继续撑大 Dashboard。变更文件栏
在路径宽于栏位时也允许横向滚动，文件和文件夹名称不会被省略。变更文件按文件夹分组，文件夹默认展开，
并且可以独立展开或折叠。Diff 内容也会在固定尺寸的 Diff 栏内滚动；只读选择和刷新行为保持不变。
