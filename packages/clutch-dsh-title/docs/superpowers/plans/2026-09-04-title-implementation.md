...
2026-09-11 修订：在 `wt-title-0.1.3/feat-optimize-prompt-thinking` 中，
依据 DSH session `session-b555e652-640c-4d6d-b4c0-ae4c843e9e1b` 补齐
plugin-only 请求参数控制。Cordis 新增 `reasoningEffort`，默认 `off`，
接受适配器定义的非空 ID；显式 `null` 表示省略参数。通过公开
`ReasoningEffortId` / `GenerateOptions.reasoningEffort` 传递，仅作用于模板
extraction，管理模板切换不能覆盖此配置。保留现有 prompt 与原生 request-event
schema，不修改 DSH 源码、主会话或禁用模板时的原生生成器。适配器决定实际语义，
尤其 pi-ai 的 `off` 可能仅省略 reasoning，不能承诺所有模型关闭思考；
不支持的强度沿用原生失败 fallback，不自动换强度重试。
