...
- Set every DSH peer range to `>=0.1.7-rc.1`, removing the legacy `>=0.1.2-rc.1` constraint as DSH 0.1.2-rc.1 can no longer run this plugin; broaden the Cordis peer from exact 4.0.1 to `^4.0.1` so the DSH release's 4.0.4 peer is accepted. Pin development-time DSH dependencies to `0.1.7-rc.1`, Cordis to 4.0.4, and Schemastery to `^3.18.4` so checks compile and run against the target API.
